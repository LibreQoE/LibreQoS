...
RUSTPROGS="lqosd lqos_node_manager lqtop xdp_iphash_to_cpu_cmdline xdp_pping lqusers lqos_setup lqos_map_perf uisp_integration lqos_support_tool"
...
echo "/bin/systemctl enable lqos_node_manager" >> postinst # In case it's running from a previous release
echo "/bin/systemctl start lqos_node_manager" >> postinst # In case it's running from a previous release
