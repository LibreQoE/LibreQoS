...
class NodeType(enum.IntEnum):
    """Enumeration to define what type of node a NetworkNode is."""
...
class NetworkNode:
    ...
    def __init__(
        ...
        displayName: str = "",
        parentId: str = "",
        siteType: NodeType = NodeType.site,
        download: int = generatedPNDownloadMbps,
        upload: int = generatedPNUploadMbps,
        ipv4: List = [],
        ipv6: List = [],
        address: str = "",
        mac: str = "",
...
class NetworkGraph:
    ...
    def __init__(self) -> None:
        ...
        self.nodes = [
            NetworkNode(
                "FakeRoot", siteType=NodeType.root, parentId="", displayName="Shaper Root"
            )
        ]
    ...
    def addNodeAsChild(self, parent: str, node: NetworkNode) -> None:
        ...
        meant it.
        """
        if node.displayName in self.excludeSites:
            return
        parentIdx = 0
    ...
    def __promoteClientsWithChildren(self) -> None:
        ...
        """
        for i, node in enumerate(self.nodes):
            if node.siteType == NodeType.client:
                for child in self.findChildIndices(i):
                    if self.nodes[child].siteType != NodeType.device:
                        node.siteType = NodeType.clientWithChildren

    def __clientsWithChildrenToSites(self) -> None:
        toAdd = []
        for i, node in enumerate(self.nodes):
            if node.siteType == NodeType.clientWithChildren:
                siteNode = NetworkNode(
                    siteId=node.id + "_gen",
                    displayName="(Generated Site) " + node.displayName,
                    siteType=NodeType.site,
                )
                siteNode.parentIndex = node.parentIndex
                node.parentId = siteNode.siteId
                if node.siteType == NodeType.clientWithChildren:
                    node.siteType = NodeType.client
                for child in self.findChildIndices(i):
                    if (
                        self.nodes[child].siteType == NodeType.client
                        or self.nodes[child].siteType == NodeType.clientWithChildren
                        or self.nodes[child].siteType == NodeType.site
                    ):
                        self.nodes[child].parentId = siteNode.siteId
    ...
    def __isSite(self, index) -> bool:
        return (
            self.nodes[index].siteType == NodeType.ap
            or self.nodes[index].siteType == NodeType.site
            or self.nodes[index].siteType == NodeType.clientWithChildren
        )
    ...
    def createNetworkJson(self):
            ...
            for node in data:
                if isinstance(node, str):
                    if (isinstance(data[node], dict)) and (node != "children"):
                        data[node]["downloadBandwidthMbps"] = min(
                            int(data[node]["downloadBandwidthMbps"]), int(parentMaxDL)
                        )
                        data[node]["uploadBandwidthMbps"] = min(
                            int(data[node]["uploadBandwidthMbps"]), int(parentMaxUL)
                        )
        ...
        inheritBandwidthMaxes(
            topLevelNode,
            parentMaxDL=upstreamBandwidthCapacityDownloadMbps,
    ...
    def __buildNetworkObject(self, idx):
        ...
        node = {
            ...
            "type": nodeTypeToString(self.nodes[idx].siteType),
        }
        children = {}
        hasChildren = False
            ...
            if (
                ...
                for child in children:
                    if child != i:
                        if (
                            self.nodes[child].siteType != NodeType.client
                            and self.nodes[child].siteType != NodeType.device
                        ) or showClients:
