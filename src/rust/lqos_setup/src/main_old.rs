...
fn get_bandwidth(up: bool) -> u64 {
    loop {
        match up {
            true => println!(
                "{}",
                "How much UPLOAD bandwidth do you have? (Mbps, e.g. 1000 = 1 gbit)".yellow()
            ),
            false => println!(
                "{}",
                "How much DOWNLOAD bandwidth do you have? (Mbps, e.g. 1000 = 1 gbit)".yellow()
            ),
        }
...
fn anonymous() -> bool {
    println!(
        "{}",
        "Help Improve LibreQoS with Anonymous Statistics?".yellow()
    );
    println!(
        "{}",
        "We'd really appreciate it if you'd allow anonymous statistics".green()
    );
    println!(
        "{}",
        "to be sent to our way. They help us focus our develpoment,".green()
    );
