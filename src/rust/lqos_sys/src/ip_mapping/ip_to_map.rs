...
impl IpToMap {
  pub(crate) fn new(
    ...
    Ok(Self { subnet, prefix: subnet_part, tc_handle, cpu })
  }
...
mod test {
  ...
  fn parse_ipv4_single() {
    let map =
      IpToMap::new("1.2.3.4", TcHandle::from_string("1:2").unwrap(), 1)
        .unwrap();
  ...
  fn parse_ipv4_subnet() {
    let map =
      IpToMap::new("1.2.3.0/24", TcHandle::from_string("1:2").unwrap(), 1)
        .unwrap();
  ...
  fn parse_ipv4_invalid_ip() {
    let map =
      IpToMap::new("1.2.3.256/24", TcHandle::from_string("1:2").unwrap(), 1);
    assert!(map.is_err());
  }
  ...
  fn parse_ipv4_super_invalid_ip() {
    let map =
      IpToMap::new("I like sheep", TcHandle::from_string("1:2").unwrap(), 1);
    assert!(map.is_err());
  }
  ...
  fn parse_ipv4_invalid_cidr() {
    let map =
      IpToMap::new("1.2.3.256/33", TcHandle::from_string("1:2").unwrap(), 1);
    assert!(map.is_err());
  }
  ...
  fn parse_ipv4_negative_cidr() {
    let map =
      IpToMap::new("1.2.3.256/-1", TcHandle::from_string("1:2").unwrap(), 1);
    assert!(map.is_err());
  }
  ...
  fn parse_ipv6_single() {
    let map =
      IpToMap::new("dead::beef", TcHandle::from_string("1:2").unwrap(), 1)
        .unwrap();
