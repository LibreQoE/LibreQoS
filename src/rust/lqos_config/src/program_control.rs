...
fn path_to_libreqos() -> Result<PathBuf, ProgramControlError> {
    let cfg = etc::EtcLqos::load().map_err(|_| ProgramControlError::ConfigLoadError)?;
...
pub fn load_libreqos() -> Result<String, ProgramControlError> {
    ...
    let stdout = String::from_utf8(result.stdout).map_err(|_| ProgramControlError::StdInErrAccess)?;
    let stderr = String::from_utf8(result.stderr).map_err(|_| ProgramControlError::StdInErrAccess)?;
    Ok(stdout + &stderr)
}
