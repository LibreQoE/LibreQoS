...
pub(crate) struct SerializableShapedDevice {
    ...
    pub comment: String,
    /// Optional per-circuit SQM override: "cake", "fq_codel", "none", or "down_sqm/up_sqm".
    /// Empty = default.
    pub sqm: String,
}
...
impl From<&ShapedDevice> for SerializableShapedDevice {
    fn from(d: &ShapedDevice) -> Self {
        Self {
            ...
            parent_node_id: d.parent_node_id.clone().unwrap_or_default(),
            anchor_node_id: d.anchor_node_id.clone().unwrap_or_default(),
            mac: d.mac.clone(),
