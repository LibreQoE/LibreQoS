...
#[cfg(feature = "async")]
pub async fn receive_payload<T: DeserializeOwned>(
    our_keys: &KeyStore,
    their_public_key: &PublicKey,
    ...
    let nonce_size = tcp_stream.read_u32().await?;
    // Receive the nonce
    let mut nonce_bytes = vec![0; nonce_size as usize];
    tcp_stream.read_exact(&mut nonce_bytes).await?;
    ...
    let encrypted_size = tcp_stream.read_u64().await?;
    // Receive the encrypted payload
    let mut encrypted_bytes = vec![0; encrypted_size as usize];
    tcp_stream.read_exact(&mut encrypted_bytes).await?;
