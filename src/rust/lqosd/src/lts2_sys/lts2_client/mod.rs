...
pub fn spawn_lts2() -> anyhow::Result<()> {
    ...
    if let Err(e) = std::thread::Builder::new().name("License Check".to_string())
        .spawn(move || {
...
pub fn request_free_trial(
    ...
    if let Ok(tx) = client_commands::get_command_channel() {
        ...
        if tx.send(LtsClientCommand::RequestFreeTrial(details, otx)).is_err() {
            println!("Failed to send free trial request to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send free trial request to LTS2 client"));
        }
        return if let Ok(result) = orx.blocking_recv() {
            Ok(result)
        } else {
            println!("Failed to receive free trial response from LTS2 client");
            Err(anyhow::anyhow!("Failed to receive free trial response from LTS2 client"))
        }
    }
    Err(anyhow::anyhow!("Failed to get command channel"))
}

pub fn submit_network_tree(
    timestamp: u64,
    tree: &[u8]
) -> anyhow::Result<()> {
    if let Ok(tx) = client_commands::get_command_channel() {
        let tree = tree.to_vec();
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::NetworkTree {
            timestamp,
            tree,
        })).is_err() {
            println!("Failed to send network tree to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send network tree to LTS2 client"));
        }
    }
    Ok(()) // SUCCESS
}

pub fn submit_shaped_devices(
    timestamp: u64,
    devices: &[u8]
) -> anyhow::Result<()> {
    let devices = devices.to_vec();
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::ShapedDevices {
            timestamp,
            devices,
        })).is_err() {
            println!("Failed to send shaped devices to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send shaped devices to LTS2 client"));
        }
    }
    Ok(()) // SUCCESS
}
...
pub fn submit_total_throughput(
    ...
    if let Ok(tx) = client_commands::get_command_channel() {
        ...
        let median_rtt = if has_median_rtt { Some(median_rtt) } else { None };
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::TotalThroughput {
            timestamp,
            download_bytes,
            upload_bytes,
            ...
            return Err(anyhow::anyhow!("Failed to send total throughput to LTS2 client"));
        }
    } else {
...
pub fn submit_shaper_utilization(
    ...
    if let Ok(tx) = client_commands::get_command_channel() {
        if let Err(e) = tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::ShaperUtilization {
            tick: timestamp,
            average_cpu,
            peak_cpu,
            memory_percent,
            ...
            return Err(anyhow::anyhow!("Failed to send shaper utilization to LTS2 client"));
        }
    }
    Ok(()) // SUCCESS
}
...
pub fn submit_circuit_throughput_batch(
    ...
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::CircuitThroughputBatch(
            batch.to_vec()
        ))).is_err() {
            error!("Failed to send circuit throughput batch to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send circuit throughput batch to LTS2 client"));
        }
    }
    Ok(()) // SUCCESS
}

pub fn submit_circuit_retransmits_batch(
    batch: &[CircuitRetransmits]
) -> anyhow::Result<()> {
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::CircuitRetransmitsBatch(
            batch.to_vec()
        ))).is_err() {
            println!("Failed to send circuit retransmits batch to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send circuit retransmits batch to LTS2 client"));
        }
    }
    Ok(()) // SUCCESS
}
...
pub fn submit_circuit_rtt_batch(
    ...
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::CircuitRttBatch(
            batch.to_vec()
        ))).is_err() {
            error!("Failed to send circuit RTT batch to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send circuit RTT batch to LTS2 client"));
        }
    }
    Ok(()) // SUCCESS
}
...
pub fn submit_circuit_cake_drops_batch(
    ...
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::CircuitCakeDropsBatch(
            batch.to_vec()
        ))).is_err() {
            println!("Failed to send circuit cake drops batch to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send circuit cake drops batch to LTS2 client"));
        }
    }
    Ok(()) // SUCCESS
}
...
pub fn submit_circuit_cake_marks_batch(
    ...
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::CircuitCakeMarksBatch(
            batch.to_vec()
        ))).is_err() {
            println!("Failed to send circuit cake marks batch to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send circuit cake marks batch to LTS2 client"));
        }
    }
    Ok(()) // SUCCESS
}

pub fn submit_site_throughput_batch(
    batch: &[shared_types::SiteThroughput]
) -> anyhow::Result<()> {
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::SiteThroughputBatch(
            batch.to_vec()
        ))).is_err() {
            error!("Failed to send site throughput batch to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send site throughput batch to LTS2 client"));
        }
    }
    Ok(()) // SUCCESS
}
...
pub fn submit_site_retransmits_batch(
    ...
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::SiteRetransmitsBatch(
            batch.to_vec()
        ))).is_err() {
            error!("Failed to send site retransmits batch to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send site retransmits batch to LTS2 client"));
        }
    }
    Ok(()) // SUCCESS
}

pub fn submit_site_cake_drops_batch(
    batch: &[shared_types::SiteCakeDrops]
) -> anyhow::Result<()> {
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::SiteCakeDropsBatch(
            batch.to_vec()
        ))).is_err() {
            println!("Failed to send site cake drops batch to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send site cake drops batch to LTS2 client"));
        }
    }
    Ok(()) // SUCCESS
}

pub fn submit_site_cake_marks_batch(
    batch: &[shared_types::SiteCakeMarks]
) -> anyhow::Result<()> {
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::SiteCakeMarksBatch(
            batch.to_vec()
        ))).is_err() {
            println!("Failed to send site cake marks batch to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send site cake marks batch to LTS2 client"));
        }
    }
    Ok(()) // SUCCESS
}

pub fn submit_site_rtt_batch(
    batch: &[shared_types::SiteRtt]
) -> anyhow::Result<()> {
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::SiteRttBatch(
            batch.to_vec()
        ))).is_err() {
            println!("Failed to send site RTT batch to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send site RTT batch to LTS2 client"));
        }
    }
    Ok(()) // SUCCESS
}
...
pub fn ingest_batch_complete() -> anyhow::Result<()> {
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestBatchComplete).is_err() {
            ...
            return Err(anyhow::anyhow!("Failed to send ingest batch complete message to LTS2 client"))
        }
    }
    Ok(())
}
...
pub fn one_way_flow(
    ...
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::OneWayFlow {
            start_time,
            end_time,
            local_ip,
            remote_ip,
            ...
            return Err(anyhow::anyhow!("Failed to send one-way flow to LTS2 client"));
        }
    }
    Ok(())
}
...
{ 
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::TwoWayFlow {
            start_time,
            end_time,
            local_ip,
            remote_ip,
...
pub fn allow_subnet(ip_string: String) -> anyhow::Result<()> {
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::AllowSubnet(ip_string))).is_err() {
            error!("Failed to send allow subnet to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send allow subnet to LTS2 client"));
        }
    }
    Ok(())
}
...
pub fn ignore_subnet(ip_string: String) -> anyhow::Result<()> {
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::IgnoreSubnet(ip_string))).is_err() {
            error!("Failed to send ignore subnet to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send ignore subnet to LTS2 client"));
        }
    }
    Ok(())
}
...
pub fn submit_blackboard(bytes: &[u8]) -> anyhow::Result<()> {
    ...
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::BlackboardJson(bytes))).is_err() {
            error!("Failed to send blackboard JSON to LTS2 client");
            return Err(anyhow::anyhow!("Failed to send blackboard JSON to LTS2 client"));
        }
    }
    Ok(())
}
...
pub fn flow_count(timestamp: u64, flow_count: u64) -> anyhow::Result<()> {
    if let Ok(tx) = client_commands::get_command_channel() {
        if tx.send(LtsClientCommand::IngestData(ingestor::commands::IngestorCommand::FlowCount{ timestamp, flow_count })).is_err() {
...
pub fn get_remote_host() -> String {
    if let Ok(config) = load_config() {
        return config.long_term_stats.lts_url.clone().unwrap_or("insight.libreqos.com".to_string());
    }
