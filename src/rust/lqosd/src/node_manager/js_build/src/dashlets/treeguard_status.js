...
export class TreeGuardStatusDashlet extends BaseDashlet {
    ...
    _rerenderWithEstimates() {
        if (this.lastStatusData) {
            this._renderStatus(this.lastStatusData);
        }
    }
    ...
    _renderStatus(d) {
        ...
        const managedNodes = Math.max(0, Math.trunc(Number(d.managed_nodes ?? 0) || 0));
        const managedCircuits = Math.max(0, Math.trunc(Number(d.managed_circuits ?? 0) || 0));

        const virtualizedNodesExact = Math.max(0, Math.trunc(Number(d.virtualized_nodes ?? 0) || 0));
        const fqCodelCircuitsExact = Math.max(0, Math.trunc(Number(d.fq_codel_circuits ?? 0) || 0));
