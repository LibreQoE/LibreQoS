...
///
/// This function is not pure: it reads the current configuration from disk (via the config cache).
...
///
/// This function is pure with respect to Autopilot behavior: it does not trigger actions.
/// In the current scaffolding, it returns an empty list.
