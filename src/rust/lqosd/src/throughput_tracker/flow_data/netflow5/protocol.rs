...
pub(crate) fn to_netflow_5(
    ...
    if let (IpAddr::V4(local), IpAddr::V4(remote)) = (local, remote) {
        ...
        let d_pkts2 = (data.packets_enqueued.down as u32).to_be();
        let d_octets2 = (data.bytes_enqueued.down as u32).to_be();
        let d_pkts = (data.packets_enqueued.up as u32).to_be();
        let d_octets = (data.bytes_enqueued.up as u32).to_be();
