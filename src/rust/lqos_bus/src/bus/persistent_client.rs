...
use crate::{
    BUS_SOCKET_PATH, BusRequest, BusResponse, BusSession, decode_response, encode_request,
};
...
impl BusClient {
    ...
    pub async fn request(
        ...
        if self.stream.is_some() && self.stream.as_ref().unwrap().writable().await.is_err() {
