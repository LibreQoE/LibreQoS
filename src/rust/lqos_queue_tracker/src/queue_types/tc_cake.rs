        ...
        match value {
            Value::Object(map) => {
                ...
                for (key, value) in map.iter() {
                    match key.as_str() {
                        "bandwidth" => {
                            result.bandwidth = BandWidth::from_str(value.as_str().unwrap_or(""))
                        }
                        "diffserv" => {
                            result.diffserv = DiffServ::from_str(value.as_str().unwrap_or(""))
                        }
                        "flowmode" => {
                            result.flowmode = FlowMode::from_str(value.as_str().unwrap_or(""))
                        }
                        ...
                        "ack-filter" => {
                            result.ack_filter = AckFilter::from_str(value.as_str().unwrap_or(""))
                        }
                        ...
                        _ => {
                            info!("Unknown entry in tc-cake-options json decoder: {key}");
                        }
                    }
                }
                Ok(result)
