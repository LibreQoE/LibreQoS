...
def extractServiceIPs(serviceItem, allocated_ipv4s, allocated_ipv6s, customer_name):
	"""Extract and validate IPv4/IPv6 addresses from a service item."""
	ipv4_list = []
	ipv6_list = []
	...
	if serviceItem['ipv4'] != '':
		if serviceItem['ipv4'] not in allocated_ipv4s:
			...
			print(f"Client {customer_name} had duplicate IP of {serviceItem['ipv4']}. IP omitted.")
		...
		for ipv4_route in serviceItem['ipv4_route'].split(', '):
			if ipv4_route and ipv4_route.strip():
				if ipv4_route not in allocated_ipv4s:
					...
					print(f"Client {customer_name} had duplicate IPv4 route of {ipv4_route}. IP omitted.")
	...
	if serviceItem['ipv6'] != '':
		if serviceItem['ipv6'] not in allocated_ipv6s:
			...
			print(f"Client {customer_name} had duplicate IP of {serviceItem['ipv6']}. IP omitted.")
		...
		for ipv6_delegation in serviceItem['ipv6_delegated'].split(', '):
			if ipv6_delegation and ipv6_delegation.strip():
				if ipv6_delegation not in allocated_ipv6s:
					...
					print(f"Client {customer_name} had duplicate IPv6 delegation of {ipv6_delegation}. IP omitted.")
	
	return ipv4_list, ipv6_list

def createClientAndDevice(net, serviceItem, customer_name, downloadForTariffID, uploadForTariffID, 
						  ipv4_list, ipv6_list, parent_id, device_counter):
	"""Create client circuit and device nodes."""
...
def createShaperFlat(net, allServices, customers, downloadForTariffID, uploadForTariffID):
	...
	cust_id_to_name = {customer['id']: customer['name'] for customer in customers}
	...
	for serviceItem in allServices:
		if serviceItem['status'] == 'active':
			customer_name = cust_id_to_name.get(serviceItem['customer_id'], str(serviceItem['customer_id']))
			ipv4_list, ipv6_list = extractServiceIPs(serviceItem, allocated_ipv4s, allocated_ipv6s, customer_name)
			
			if ipv4_list or ipv6_list:
				device_counter = createClientAndDevice(net, serviceItem, customer_name, downloadForTariffID, 
													   uploadForTariffID, ipv4_list, ipv6_list, None, device_counter)

def createInfrastructureNodes(net, monitoring, siteBandwidth, node_types=None):
	"""Create infrastructure nodes (sites and/or APs) based on monitoring data."""
	if node_types is None:
		node_types = ['Site', 'AP']  # Create both by default
	...
	# Create nodes by type priority (Sites first, then APs)
	for node_type in ['Site', 'AP']:
		if node_type not in node_types:
			continue
			
		for monitored_device in monitoring:
			device_type = 'AP' if monitored_device.get('type') == 5 else 'Site'
			if device_type != node_type:
				continue
				
			device_id = monitored_device['id']
			device_name = monitored_device['title']
			
			# Determine parent
			parent_id = hardware_parent.get(device_id)
			if node_type == 'AP' and parent_id not in created_nodes:
				parent_id = None  # AP without valid site parent becomes orphan
			elif node_type == 'Site':
				parent_id = None  # Sites are always top-level in simplified modes
			
			# Get bandwidth
			download = upload = 10000
			...
			node = NetworkNode(
				...
				type=NodeType.ap if device_type == 'AP' else NodeType.site,
				parentId=parent_id,
				download=download,
				upload=upload,
				address=None
			...
			net.addRawNode(node)
			created_nodes[device_id] = node
	
	return created_nodes

def findParentNode(serviceItem, available_nodes):
	"""Find the best parent node for a service from available infrastructure nodes."""
	# Try access_device first, then router_id
	for field in ['access_device', 'router_id']:
		if field in serviceItem and serviceItem[field] != 0:
			if serviceItem[field] in available_nodes:
				return serviceItem[field]
	return None

def createShaperApOnly(net, allServices, customers, downloadForTariffID, uploadForTariffID, monitoring, siteBandwidth):
	"""AP Only strategy: gives you a single layer of AP + Clients."""
	print("Using ap_only strategy - single layer AP + clients")
	
	# Create only AP nodes
	ap_nodes = createInfrastructureNodes(net, monitoring, siteBandwidth, ['AP'])
	cust_id_to_name = {customer['id']: customer['name'] for customer in customers}
	device_counter = 200000
	allocated_ipv4s = {}
	allocated_ipv6s = {}
	...
	for serviceItem in allServices:
		if serviceItem['status'] == 'active':
			...
			if ipv4_list or ipv6_list:
				parent_ap_id = findParentNode(serviceItem, ap_nodes)
				device_counter = createClientAndDevice(net, serviceItem, customer_name, downloadForTariffID, 
													   uploadForTariffID, ipv4_list, ipv6_list, parent_ap_id, device_counter)

def createShaperApSite(net, allServices, customers, downloadForTariffID, uploadForTariffID, monitoring, siteBandwidth):
	"""AP Site strategy: gives you Site - AP - Clients structure."""
	print("Using ap_site strategy - Site > AP > clients")
	
	# Create both Sites and APs with proper hierarchy
	all_nodes = createInfrastructureNodes(net, monitoring, siteBandwidth, ['Site', 'AP'])
	ap_nodes = {k: v for k, v in all_nodes.items() if v.type == NodeType.ap}
