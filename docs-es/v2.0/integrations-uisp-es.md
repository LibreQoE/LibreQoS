...
Las versiones actuales también manejan de forma específica los AP AirMax donde UISP reporta `identification.type == "airMax"` y `identification.role == "ap"`. Cuando UISP entrega capacidad agregada de flexible framing (`theoreticalTotalCapacity`), LibreQoS la divide usando `dlRatio` si está disponible; si no, usa `airmax_flexible_frame_download_ratio`, cuyo valor por defecto `0.8` significa 80/20 descarga/subida.
