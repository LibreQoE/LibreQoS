...
```{important}
Starting in v2.0, mapped-circuit ingest depends on a [valid license](https://libreqos.com/pricing). Without a valid Insight or Local license, LibreQoS reads only the first 1000 valid mapped circuits into active shaping state. See [Insight Licensing Behavior](insight.md#mapped-circuit-limits-and-license-state) and [Troubleshooting](troubleshooting.md). The Node API uses the same mapped-circuit policy: networks with 1,000 or fewer valid mapped circuits may use the API without an Insight subscription; larger networks require a valid license.
```
